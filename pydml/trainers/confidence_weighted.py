...
class ConfidenceWeightedDML(BaseCollaborativeTrainer):
    ...
    def get_confidence_stats(self) -> Dict:
        ...
        for i in range(len(self.models)):
            stats[f"model_{i}_avg_confidence"] = sum(self.confidence_history[i]) / len(
                self.confidence_history[i]
            )
            stats[f"model_{i}_avg_weight"] = sum(self.weight_history[i]) / len(
                self.weight_history[i]
            )
...
def compare_standard_vs_confidence_weighted(
    ...
    config = ConfidenceWeightedConfig(
        confidence_threshold=0.6, weighting_strategy="softmax", adaptive=True
    )
    ...
    comparison = {
        ...
        "improvement": results_weighted["avg_acc"][-1]
        - results_standard["avg_acc"][-1],
