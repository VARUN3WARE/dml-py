...
class DMLTrainer(BaseCollaborativeTrainer):
    ...
    def __init__(
        ...
        # 1. models must be list or tuple
        if not isinstance(models, (list, tuple)):
            raise TypeError(
                f"`models` must be a list or tuple of nn.Module, got {type(models)}"
            )

        # 2. At least two models required
        if len(models) < 2:
            raise ValueError(
                "DMLTrainer requires at least two models for Deep Mutual Learning."
            )

        # 3. All items must be nn.Module
        for i, model in enumerate(models):
            if not isinstance(model, nn.Module):
                raise TypeError(
                    f"Model at index {i} is not an nn.Module, got {type(model)}"
                )

        # 4. Output dimensions must match (static heuristic)
        def infer_output_dim(model: nn.Module):
            for module in reversed(list(model.modules())):
                if isinstance(module, nn.Linear):
                    return module.out_features
            return None

        output_dims = [infer_output_dim(m) for m in models]

        if any(dim is None for dim in output_dims):
            raise ValueError(
                "Unable to infer output dimension for one or more models. "
                "Ensure all models have a final nn.Linear classification head."
            )

        if len(set(output_dims)) != 1:
            raise ValueError(
                f"All models must have the same output dimension, got {output_dims}"
            )
        ...
        print(
            f"Config: temperature={self.config.temperature}, "
