...
class FeatureDMLTrainer(BaseCollaborativeTrainer):
    ...
    def __init__(
        ...
        if feature_layers_list is not None:
            for i, (model, layer_names) in enumerate(
                zip(self.models, feature_layers_list)
            ):
        ...
        if self.config.feature_loss_type == "mse":
            ...
            raise ValueError(
                f"Unknown feature loss type: {self.config.feature_loss_type}"
            )

        print(f"Feature-Based DML Trainer initialized with {self.num_models} models")
        print(
            f"Config: temp={self.config.temperature}, "
    ...
    def _match_feature_dimensions(self, features1, features2):
        ...
        if len(features1.shape) == 4:  # Conv features (B, C, H, W)
            ...
            if features1.size(2) != target_size or features1.size(3) != target_size:
                features1 = nn.functional.adaptive_avg_pool2d(
                    features1, (target_size, target_size)
                )
            if features2.size(2) != target_size or features2.size(3) != target_size:
                features2 = nn.functional.adaptive_avg_pool2d(
                    features2, (target_size, target_size)
                )
    ...
    def train_epoch(self, train_loader: DataLoader, epoch: int) -> Dict[str, float]:
        ...
        metrics = {
            ...
            / (self.num_models * len(train_loader)),
            "train_acc": sum(total_correct.values())
            / (self.num_models * total_samples)
            * 100,
        }

        for i in range(self.num_models):
            metrics[f"train_loss_model_{i}"] = epoch_losses[f"model_{i}"] / len(
                train_loader
            )
            metrics[f"train_acc_model_{i}"] = (
                total_correct[f"model_{i}"] / total_samples * 100
            )
