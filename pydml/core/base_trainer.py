...
class BaseCollaborativeTrainer(ABC):
    ...
    def __init__(
        ...
        if optimizers is None:
            self.optimizers = [
                torch.optim.Adam(model.parameters(), lr=0.001) for model in self.models
            ]
        else:
            assert (
                len(optimizers) == self.num_models
    ...
    def fit(
        ...
        for epoch in range(1, epochs + 1):
            ...
            if verbose:
                ...
                print(
                    f"  Train Loss: {train_metrics['train_loss']:.4f} | Train Acc: {train_metrics['train_acc']:.2f}%"
                )
                if val_loader is not None:
                    print(
                        f"  Val Loss: {val_metrics['val_loss']:.4f} | Val Acc: {val_metrics['val_acc']:.2f}%"
                    )

                # Per-model stats
                for i in range(self.num_models):
                    print(
                        f"  Model {i}: Train Acc: {train_metrics[f'train_acc_model_{i}']:.2f}%",
                        end="",
                    )
                    if val_loader is not None:
