...
def ensemble_accuracy(outputs_list: List[torch.Tensor], targets: torch.Tensor) -> float:
    """
...
def diversity_score(outputs_list: List[torch.Tensor]) -> float:
    """
...
def agreement_matrix(outputs_list: List[torch.Tensor]) -> np.ndarray:
    """
