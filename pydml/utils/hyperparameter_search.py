...
class HyperparameterSearcher:
    ...
    def plot_results(self, output_path: str):
        ...
        best_idx = (
            np.argmax(metrics) if self.direction == "maximize" else np.argmin(metrics)
        )
        plt.scatter(
            [best_idx],
            [metrics[best_idx]],
    ...
    def search(self, space: HyperparameterSpace, verbose: bool = True) -> Dict:
        ...
        for i, config in enumerate(configs):
            try:
                ...
                self.results.append({"trial": i, "config": config, "metric": metric})

                if verbose:
                    print(
                        f"Trial {i+1}/{len(configs)}: {self.metric_name}={metric:.4f}"
                    )
...
class RandomSearcher(HyperparameterSearcher):
    ...
    def search(
        self, space: HyperparameterSpace, n_trials: int = 20, verbose: bool = True
    ) -> Dict:
        """Run random search."""
...
class OptunaSearcher(HyperparameterSearcher):
    ...
    def search(
        self, n_trials: int = 50, timeout: Optional[int] = None, verbose: bool = True
    ) -> Dict:
        ...
        def objective(trial):
            ...
            self.results.append(
                {"trial": trial.number, "config": config, "metric": metric}
            )

            return metric

        study = optuna.create_study(
            direction=self.direction, sampler=optuna.samplers.TPESampler()
        )
        ...
        study.optimize(
            objective, n_trials=n_trials, timeout=timeout, show_progress_bar=verbose
        )
