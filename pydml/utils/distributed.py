...
class DistributedManager:
    ...
    def wrap_model(
        self, model: torch.nn.Module, device_ids: Optional[List[int]] = None
    ) -> torch.nn.Module:
        """
...
def launch_distributed(
    ...
    **kwargs
    ...
    mp.spawn(
        train_fn, args=(config,) + tuple(kwargs.values()), nprocs=nprocs, join=True
    )


def apply_distributed_to_trainer(
    trainer, dist_config: Optional[DistributedConfig] = None
):
    """
