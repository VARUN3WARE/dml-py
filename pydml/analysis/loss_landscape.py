...
class LossLandscape:
    ...
    def perturb_parameters(self, direction: List[torch.Tensor], alpha: float):
        ...
        Args:
            ...
            alpha: Step size
        """
        for param, orig, d in zip(
            self.model.parameters(), self.original_params, direction
        ):
    ...
    def plot_2d(
        ...
        for i, alpha in enumerate(alphas):
            for j, beta in enumerate(betas):
                # Combine directions
                combined_dir = [
                    d1 * alpha + d2 * beta for d1, d2 in zip(direction1, direction2)
                ]
    ...
    def plot_trajectory(
        ...
        for checkpoint in checkpoints:
            ...
            delta = [
                p - orig
                for p, orig in zip(self.model.parameters(), self.original_params)
            ]
        ...
        plt.scatter(
            projections[:, 1],
            projections[:, 0],
            c=range(len(projections)),
            cmap="viridis",
            s=50,
            zorder=5,
        )
        plt.scatter([0], [0], color="red", s=200, marker="*", label="Start", zorder=10)
        plt.scatter(
            [projections[-1, 1]],
            [projections[-1, 0]],
            color="green",
...
def quick_landscape_analysis(
    ...
    landscape.plot_2d(
        output_dir / "2d_landscape_contour.png",
        direction1=dir1,
        direction2=dir2,
        num_points=20,
    )
    landscape.plot_2d(
        output_dir / "2d_landscape_3d.png",
        direction1=dir1,
        direction2=dir2,
        num_points=20,
        plot_3d=True,
...
__all__ = ["LossLandscape", "quick_landscape_analysis"]
