...
def plot_training_history(
    history: Dict[str, List[float]], save_path: Optional[str] = None, show: bool = True
):
    """
...
def plot_model_comparison(
    ...
    plt.title(
        f'Model-wise {metric.replace("_", " ").title()}', fontsize=14, fontweight="bold"
    )
...
def plot_entropy_distribution(
    ...
    plt.axvline(
        ...
        linestyle="--",
        linewidth=2,
        label=f"Mean: {entropies_np.mean():.3f}",
    )
...
def create_training_dashboard(
    history: Dict[str, List[float]], num_models: int, save_path: Optional[str] = None
):
    """
    ...
    if final_accs:
        ...
        bars = ax4.bar(
            x, final_accs, color=colors[: len(final_accs)], alpha=0.7, edgecolor="black"
        )
        ax4.axhline(
            np.mean(final_accs),
            color="red",
            linestyle="--",
            linewidth=2,
            label=f"Mean: {np.mean(final_accs):.2f}%",
        )
