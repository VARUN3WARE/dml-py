...
class PeerSelector(ABC):
    ...
    def get_selection_mask(
        self, model_idx: int, num_models: int, peer_indices: List[int]
    ) -> torch.Tensor:
    ...
    def get_selection_weights(
        self, model_idx: int, outputs: List[torch.Tensor], peer_indices: List[int]
    ) -> torch.Tensor:
    ...
    def select_peers(
        ...
        if metrics is None or len(metrics) == 0:
            ...
            return np.random.choice(
                candidates, size=min(current_k, len(candidates)), replace=False
            ).tolist()
            ...
            for i in range(len(selections) - 1):
                for model_idx in selections[i]:
                    if model_idx in selections[i + 1]:
                        if set(selections[i][model_idx]) != set(
                            selections[i + 1][model_idx]
                        ):
                            changes += 1
