...
class TemperatureSchedulerConfig:
    ...
    strategy: str = (
        "constant"  # constant, linear, exponential, cosine, adaptive, cyclical
    )
...
class TemperatureScheduler(ABC):
    ...
    def __init__(self, config: TemperatureSchedulerConfig):
        ...
        self.history = {"temperatures": [], "metrics": [], "adjustments": []}
...
class LinearAnnealing(TemperatureScheduler):
    ...
    def step(self, epoch: int, metrics: Optional[Dict] = None) -> float:
        ...
        if epoch < self.config.warmup_epochs:
            ...
            self.current_temp = self.config.initial_temp + progress * (
                self.config.final_temp - self.config.initial_temp
            )

        # Clamp to bounds
        self.current_temp = max(
            self.config.min_temp, min(self.config.max_temp, self.current_temp)
        )
    ...
    def step(self, epoch: int, metrics: Optional[Dict] = None) -> float:
        ...
        if epoch < self.config.warmup_epochs:
            ...
            if self.config.final_temp > 0:
                ...
                self.current_temp = self.config.initial_temp * math.exp(
                    -decay_rate * progress
                )
            else:
...
class TemperatureAnalyzer:
    ...
    def visualize_schedule(
        ...
        ax.set_title(
            "Temperature Scheduling Strategies", fontsize=14, fontweight="bold"
        )
...
if __name__ == "__main__":
    ...
    configs = {
        ...
        "Linear": TemperatureSchedulerConfig(
            strategy="linear",
            initial_temp=8.0,
            final_temp=1.0,
            warmup_epochs=10,
        ...
        "Exponential": TemperatureSchedulerConfig(
        ...
        "Cosine": TemperatureSchedulerConfig(
            strategy="cosine",
            initial_temp=8.0,
            final_temp=1.0,
            warmup_epochs=10,
        ...
        "Cyclical": TemperatureSchedulerConfig(
            strategy="cyclical", min_temp=2.0, max_temp=8.0, cycle_length=20
        ),
    }

    schedulers = {
        name: create_temperature_scheduler(config) for name, config in configs.items()
    }
    ...
    TemperatureAnalyzer.visualize_schedule(
        schedulers, total_epochs, "temperature_schedules.png"
    )
