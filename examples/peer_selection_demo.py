    ...
    def train_epoch(self, train_loader, epoch):
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            if len(self.history["val_acc"][0]) > 0:
                metrics = {
                    i: self.history["val_acc"][i][-1] for i in range(len(self.models))
                }
            else:
            ...
            for i, (model, optimizer) in enumerate(zip(self.models, self.optimizers)):
                ...
                peer_indices = self.peer_selector.select_peers(
                    i, outputs, targets, metrics
                )
                peer_weights = self.peer_selector.get_selection_weights(
                    i, outputs, peer_indices
                )
                ...
                for peer_idx, weight in zip(peer_indices, peer_weights):
                    loss_mimicry += weight * self.kl_loss(
                        outputs[i], outputs[peer_idx].detach()
                    )
...
def visualize_results(results):
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(
        [results[s]["description"].split("(")[0].strip() for s in strategies],
        rotation=15,
        ha="right",
    )
    ...
    for strategy in strategies:
        ...
        ax.plot(
            train_acc,
            label=results[strategy]["description"].split("(")[0].strip(),
            linewidth=2,
        )
    ax.set_xlabel("Epoch")
    ...
    bp = ax.boxplot(
        data_to_plot,
        labels=[results[s]["description"].split("(")[0].strip() for s in strategies],
    )
    ax.set_ylabel("Accuracy (%)")
...
def main():
    ...
    improvement = (
        results[best_strategy]["mean_accuracy"]
        - results[worst_strategy]["mean_accuracy"]
    )
