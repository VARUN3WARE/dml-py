...
def main():
    ...
    trainer_standard = DMLTrainer(models=models_standard, device=device)
    ...
    for i, (loss, acc) in enumerate(
        zip(history_standard["train_loss"], history_standard["train_acc"])
    ):
        val_acc = history_standard["val_acc"][i] if history_standard["val_acc"] else 0
        print(
            f"    Epoch {i+1}: Loss={loss:.4f}, Train Acc={acc:.2f}%, Val Acc={val_acc:.2f}%"
        )

    print("\n  Feature-Based DML:")
    for i, (loss, acc) in enumerate(
        zip(history_feature["train_loss"], history_feature["train_acc"])
    ):
