...
def main():
    ...
    models = [resnet32(num_classes=num_classes), resnet32(num_classes=num_classes)]
    ...
    # Setup optimizers
    optimizers = [
        torch.optim.SGD(model.parameters(), lr=0.01, momentum=0.9) for model in models
    ]

    # Create trainer
    print(f"\nInitializing DML trainer...")
    trainer = DMLTrainer(
        models=models, config=config, device=device, optimizers=optimizers
    )
