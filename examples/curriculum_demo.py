...
def main():
    ...
    difficulties = DifficultyEstimator.by_ensemble_agreement(
        models=models, dataset=train_subset, device=device
    )
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        curriculum_loader = DataLoader(
            curriculum_subset, batch_size=batch_size, shuffle=True, num_workers=0
        )
    ...
    history_baseline = trainer_baseline.fit(
        full_loader, epochs=num_epochs, verbose=False
    )
