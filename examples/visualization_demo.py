...
def main():
    ...
    create_training_dashboard(history, len(models), save_path="training_dashboard.png")

    # Agreement matrix
    print("  d) Computing agreement matrix...")
    agreement = plot_agreement_matrix(
        models, test_loader, device=device, save_path="agreement_matrix.png", show=False
    )

    avg_agreement = (agreement.sum() - agreement.trace()) / (
