...
def main():
    ...
    optimizers = [
        torch.optim.SGD(
            models[0].parameters(), lr=0.1, momentum=0.9, weight_decay=5e-4
        ),
        torch.optim.SGD(
            models[1].parameters(), lr=0.05, momentum=0.9, weight_decay=5e-4
        ),  # Lower LR for MobileNet
        torch.optim.SGD(
            models[2].parameters(), lr=0.1, momentum=0.9, weight_decay=5e-4
        ),
    ]
    ...
    avg_individual = sum(
        [test_metrics[f"val_acc_model_{i}"] for i in range(len(models))]
    ) / len(models)
