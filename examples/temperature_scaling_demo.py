...
def get_cifar10_loaders(batch_size=128, subset_size=5000):
    ...
    test_indices = np.random.choice(
        len(testset), min(1000, len(testset)), replace=False
    )
...
def train_with_temperature(
    model, teacher, train_loader, test_loader, scheduler, epochs=20, device="cpu"
):
    """
    ...
    history = {"train_loss": [], "test_acc": [], "temperatures": [], "train_time": []}
    ...
    for epoch in range(epochs):
        ...
        if epoch % 5 == 0:
            print(
                f"  Epoch {epoch:2d}: Loss={epoch_loss/len(train_loader):.4f}, "
...
def compare_temperature_strategies():
    ...
    strategies = {
        "Constant (T=4)": TemperatureSchedulerConfig(
            strategy="constant", initial_temp=4.0
        ),
        ...
        "Cyclical (2↔8)": TemperatureSchedulerConfig(
            strategy="cyclical", min_temp=2.0, max_temp=8.0, cycle_length=5
        ),
    ...
    for name, config in strategies.items():
        ...
        history = train_with_temperature(
            student, teacher, train_loader, test_loader, scheduler, epochs, device
        )
        ...
        print(
            f"  Final: Acc={results[name]['final_acc']:.2f}%, "
    ...
    print("=" * 80)
    print(
        f"{'Strategy':<25} {'Final Acc':<12} {'Best Acc':<12} {'Avg Temp':<12} {'Time (s)':<10}"
    )
    print("-" * 80)

    for name, data in results.items():
        print(
            f"{name:<25} {data['final_acc']:>10.2f}%  {data['best_acc']:>10.2f}%  "
    ...
    print("-" * 80)
    print(
        f"🏆 Best Strategy: {best_strategy[0]} ({best_strategy[1]['best_acc']:.2f}% accuracy)"
    )
    print("=" * 80)
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig(
            "temperature_strategies_comparison.png", dpi=300, bbox_inches="tight"
        )
    ...
    improvements = {
        name: data["best_acc"] - results["Constant (T=4)"]["best_acc"]
        for name, data in results.items()
        if name != "Constant (T=4)"
    }

    if improvements:
        best_improvement = max(improvements.items(), key=lambda x: x[1])
        print(
            f"• Best improvement: {best_improvement[0]} (+{best_improvement[1]:.2f}% over constant)"
        )
