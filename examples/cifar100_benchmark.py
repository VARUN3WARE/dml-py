...
def train_independent(model, train_loader, val_loader, device, epochs=200):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 20 == 0:
            print(
                f"Epoch {epoch}/{epochs} - Train Acc: {train_acc:.2f}% | Val Acc: {val_acc:.2f}% | Best: {best_acc:.2f}%"
            )
...
def main():
    ...
    train_loader, val_loader, test_loader = get_cifar100_loaders(
        batch_size=batch_size, num_workers=2, val_split=0.1, download=True
    )
    ...
    best_acc, history = train_independent(
        model, train_loader, val_loader, device, epochs=num_epochs
    )
    ...
    best_accs, history = train_dml(
        models, train_loader, val_loader, device, epochs=num_epochs
    )
    training_time = time.time() - start_time
    ...
    print(
        f"  DML (2 Networks): +{results['dml_2']['avg_acc'] - results['independent']['best_acc']:.2f}%"
    )
    print(
        f"  DML (3 Networks): +{results['dml_3']['avg_acc'] - results['independent']['best_acc']:.2f}%"
    )
