...
def main():
    ...
    single_trainer = DistillationTrainer(
        teacher=teacher, student=single_student, device=device
    )

    single_history = single_trainer.fit(
        train_loader=train_loader, val_loader=test_loader, epochs=num_epochs
    )
    ...
    dml_trainer = DMLTrainer(models=dml_students, device=device)
    ...
    codistill_trainer = CoDistillationTrainer(
        teacher=teacher, students=codistill_students, config=config, device=device
    )
    ...
    for i in range(len(codistill_students)):
        ...
        print(
            f"  Student {i}: {student_acc:.2f}% (gap to teacher: {gap_to_teacher:.2f}%)"
        )
